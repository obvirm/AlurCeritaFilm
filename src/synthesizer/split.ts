 ...
 *
 * Usage:
 *   npx tsx src/synthesizer/split.ts <manifest.json> <partsDir> <N|0=auto> [--narration <tts-meta.json>] [--minutes <m>]
 */
...
function main(): void {
  ...
  if (!manifestPath || !partsDir) {
    console.error("Usage: npx tsx src/synthesizer/split.ts <manifest.json> <partsDir> <N|0=auto> [--narration <tts-meta.json>] [--minutes <m>]");
    process.exit(1);
  }
