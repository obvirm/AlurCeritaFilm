...
export interface TranscribeProgressView {
  ...
  /** Optional human-readable stage label (e.g. Movie2Short pipeline stages). */
...
export class TranscribeProgressController extends EventTarget {
  ...
  private publishInferringPercent(): void {
    ...
    const timeRatio = 1 - Math.exp(-elapsed / TranscribeProgressController.INFERRING_TIME_CONSTANT_MS);
    let percent: number;
    if (status.rawProgress > 0) {
      // Movie2Short pipeline: the bar is anchored on the real stage
      // progress reported by the action, with a small creep so it
      // still moves during long stages. It never jumps past the next
      // stage anchor (creep is far smaller than stage spacing).
      const creep = 0.04 * timeRatio;
      percent = Math.min(TranscribeProgressController.INFERRING_CEILING, status.rawProgress + creep);
    } else {
      // Whisper: no real progress is surfaced in the inferring phase,
      // so fall back to the asymptotic time curve toward the ceiling.
      const floor = this.inferringFloorFor(status.initialPhase);
      const span = TranscribeProgressController.INFERRING_CEILING - floor;
      percent = floor + span * timeRatio;
    }
    this.publish({
