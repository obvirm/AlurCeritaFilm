...
export class LoadProjectAction {
  constructor(
    ...
    private readonly previewVideoResolver: PreviewVideoResolver,
  ...
  /**
   * Best-effort re-attaches the short-cut preview video (`final_short.mp4`
   * with the burned-in narration) when the project carries a preview
   * reference — so a page refresh does not throw the editor back to the
   * full source video. Falls back to the committed source video when the
   * backend artifact is unreachable.
   */
  private async restorePreviewIfAny(projectId: string): Promise<void> {
    let preview = null;
    try {
      preview = await this.repository.loadPreviewVideo(projectId);
    } catch (error) {
      console.warn('[projects] gagal membaca referensi preview:', error);
      return;
    }
    if (!preview) return;
    try {
      const resolved = await this.previewVideoResolver(preview);
      if (!resolved) return;
      const { video } = this.editorStore.snapshot();
      this.editorStore.patch({ video: { ...video, url: resolved.url, duration: resolved.duration } });
    } catch (error) {
