 ...
 * `movie` in the default foreground (the quiet prefix), `2short` in the
...
export function Wordmark({ href, onClick, size = 'md', working = false, className = '' }: WordmarkProps) {
  ...
  const content = (
    <>
      <span className="text-fg-primary">movie</span>
      <span className={capsClass}>2short</span>
  ...
  return (      <span className={`${BASE} ${sizeClass} ${className}`} aria-label="movie2short">
      {content}
    </span>
  );
}
