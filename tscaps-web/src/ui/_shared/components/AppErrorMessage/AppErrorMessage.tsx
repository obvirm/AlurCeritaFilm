...
export function AppErrorMessage({ error, isMobile = false }: AppErrorMessageProps): ReactElement {
  switch (error.name) {
    case 'UnknownAppError':                  return <GenericFailureBody error={error} isMobile={isMobile} />;
...
function GenericFailureBody({ error, isMobile }: { error: AppError; isMobile: boolean }): ReactElement {
  // Prefer the underlying technical message so the user can actually
  // act on it (e.g. "job masih berjalan" instead of a dead-end hint).
  ...
  const message =
    cause instanceof Error && cause.message.trim().length > 0
      ...
      : error.message;
  const bullets =
    message && message !== 'Unknown application error'
      ? [message, ...engineFallbackBullets(isMobile)]
      : engineFallbackBullets(isMobile);
  return (
    <ErrorBody
